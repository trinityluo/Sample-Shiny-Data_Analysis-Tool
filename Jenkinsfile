pipeline {
  ...
  stages {
    ...
    stage('Development') {
      steps {
        sshPublisher(publishers: [sshPublisherDesc(configName: 'jenkins@gcp', transfers: [sshTransfer(excludes: '', execCommand: '''sudo mkdir -p /mnt/disks/data/shiny-server/shinyapps/sample-shiny-analysis-tool
